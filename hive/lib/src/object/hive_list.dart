...
class HiveList<E extends HiveObject>
    with ListMixin<E>, DelegatingListViewMixin<E> {
  ...
  HiveList(Box box, {List<E> objects})
      : boxName = box.name,
        _keys = null,
        _delegate = [],
        _box = box {
  ...
  HiveList.lazy(this.boxName, List<dynamic> keys) : _keys = keys;
  ...
  HiveList<T> castHiveList<T extends HiveObject>() {
    if (_delegate != null) {
      return HiveList(box, objects: _delegate.cast());
    } else {
      return HiveList.lazy(boxName, _keys);
    }
  }
